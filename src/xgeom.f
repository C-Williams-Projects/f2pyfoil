...
C---- Interactive curvature table display removed for library use
      DO 30 I=1, N
        IF(I.EQ.1) THEN
...
C---- go over each point, calculating corner angle
      IF(IPRINT.EQ.2 .AND. .NOT.LQUIET) WRITE(*,1050)
      DO 30 I=2, N-1
        DX1 = X(I) - X(I-1)
        DY1 = Y(I) - Y(I-1)
        DX2 = X(I) - X(I+1)
...
C
     ...
     &         / SQRT((DX1**2 + DY1**2) * (DX2**2 + DY2**2))
        ...
        IF(IPRINT.EQ.2 .AND. .NOT.LQUIET) WRITE(*,1100)I,X(I),Y(I),ANGL
...
C------- Remove node I+1
         ...
         N = N - 1
         LCHANGE = .TRUE.
         IF(.NOT.LQUIET) WRITE(*,*) 'SCHECK segment removed at ',I
         GO TO 10
        ENDIF
C
 20   CONTINUE
