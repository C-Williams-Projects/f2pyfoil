...
C
      LU = 31
C---- build OSFILE path from XFOIL_DATA_DIR env var, or use bare filename
      OSDIR  = ' '
      DIRLEN = 0
      CALL GET_ENVIRONMENT_VARIABLE('XFOIL_DATA_DIR', OSDIR, DIRLEN)
      IF(DIRLEN .GT. 0) THEN
          OSFILE = OSDIR(1:DIRLEN) // '/osmap.dat'
      ELSE
          OSFILE = 'osmap.dat'
      ENDIF
